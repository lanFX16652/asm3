...
let transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
        user: "phonghan548@gmail.com",
        pass: "flrmycxarcmlwbpt",
    },
});
    ...
    if (req.user) {
        try {
            ...
            if (userCart) {
                ...
                const productInCartIndex = userCart.products.findIndex(p => p.product._id.toString() === productId)
    ...
    if (req.user) {
        try {
            ...
            return res.status(200).json({
                ...
                totalCartItem: userCart?.products ? userCart.products.reduce((prev, product) => {
                    return prev += product.qty
                }, 0) : 0,
                totalPrice: userCart?.products ? userCart.products.reduce((prev, product) => {
                    return prev += (product.product.price * product.qty)
                }, 0) : 0,
            })
        } catch (error) {
...
export const cartOrder = async (req, res, next) => {
    ...
    <h4>Tổng thanh toán: ${parseCurrency(totalPrice)}<h4>
        ...
        return res.status(200).json({
            order: { ...order, status: "Waiting for pay", delivery: "Waiting for processing" }
        })
    } else {
        res.status(400).json({
        ...
        try {
            ...
            res.json({
                orders: orders.map(order => {
                    ...
                    return { ...objectOrder, status: "Waiting for pay", delivery: "Waiting for processing" }
                })
            })
        } catch (error) {
            console.log(error);
...
export const getAllOrders = async (req, res) => {
    ...
    try {
        const allOrders = await orderModel.find({}).skip((page - 1) * limit).limit(limit)
        const totalOrders = await orderModel.count()
        const totalPage = totalOrders / +limit
        const allOrderTransform = allOrders.map((order) => {
            const objectOrder = order.toObject()
            return { ...objectOrder, status: "Chưa thanh toán", delivery: "Chưa vận chuyển" }
        })
